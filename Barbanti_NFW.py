...
for i in range(3):
    for j in range(3):
        ...
        #con xy=... specifico la posizione del testo dell'annotate, riportando i valori di c e logM
...
#definisco in una lista gli estremi dell'intervallo della ricchezza
estremi=[15,20,30,45,60,200]


#Per ogni intervallo calcolo il redshift, il profilo di densità medio e la deviazione standard 
#Grafico inoltre il profilo in funzione di R, con relativa dev. std.


#definisco un array di 5 elementi dove allocherò i valori medi di redshift
redshift_medio=np.zeros(5)
prof_den_medio=np.zeros((5,8)) #una matrice per i profili medi per ogni valore di R e ogni intervallo
...
for j in range(5):
    ...
    #che nel ciclo corrisponde ai valori nel j-esimo intervallo relativo
...
#apro da file le matrici delle covarianze
...
def ln_Likelihood(theta, R, dati, m_cov, z): #def. la likelihood gaussiana
    ...
    y=-(1.0/2)*np.dot(dati-modello,np.dot(np.linalg.inv(m_cov), dati-modello))
    return y 
...
#Confrontiamo i valori dei parametri ottenuti dal best fit e quelli medi dalle catene di cui
...
for i in range(5):
    for j in range(8): #prendo le varianze dalla diagonale principale della matrice delle covarianze
        chi_quadro[i] = sum((sigma(data_set_R, 10**best_fit[i,0], best_fit[i,1], redshift_medio[i]) - prof_den_medio[i])**2/(mat_cov[i,j,j]))


print("I valori ottenuti di chi quadro sono:", chi_quadro)


#In ultima analisi confronto il profilo di densità di best fit con i 100 profili
#con parametri M e c estratti dalle catene 
...
for i in range(5): #alloco per estrarre poi 100 valori casuali dalle catene
    ...
    for j in range(100):
        ...
        #calcolo i profili di densità con i valori estratti dalle catene e li alloco
    ...
    ax[i].plot(data_set_R, sigma(data_set_R, 10**best_fit[i,0], best_fit[i,1], redshift_medio[i]), color='red', lw=2) #plotto il best fit
    ax[i].set_title(f'Intervallo n°{i+1}: confronto tra profilo di best fit e profili con parametri delle catene')
    ax[i].set_ylabel('Profilo Σ [M$_\odot$ / Mpc$^2$]')
